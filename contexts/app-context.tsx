...
const sampleMoodEntries: MoodEntry[] = [
  { id: "mood-1", mood: 3, note: "Feeling okay, had some cravings but managed them", date: "2025-03-11T10:30:00Z" },
  { id: "mood-2", mood: 4, note: "Great day! Completed all my tasks", date: "2025-03-12T14:15:00Z" },
  { id: "mood-3", mood: 2, note: "Difficult day, struggled with triggers", date: "2025-03-13T18:45:00Z" },
  { id: "mood-4", mood: 3, note: "Better than yesterday, meditation helped", date: "2025-03-14T09:20:00Z" },
  { id: "mood-5", mood: 5, note: "Excellent day, feeling strong and positive", date: "2025-03-15T16:10:00Z" },
  { id: "mood-6", mood: 4, note: "Good day overall, enjoyed group session", date: "2025-03-16T11:30:00Z" },
  { id: "mood-7", mood: 3, note: "Relaxing day, some anxiety but manageable", date: "2025-03-17T20:00:00Z" },
]
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const storedMoodEntries = localStorage.getItem("neuroReset_moodEntries")
        const storedJournalEntries = localStorage.getItem("neuroReset_journalEntries")
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      localStorage.setItem("neuroReset_moodEntries", JSON.stringify(moodEntries))
      localStorage.setItem("neuroReset_journalEntries", JSON.stringify(journalEntries))
  ...
  const updateJournalEntry = (id: string, entryUpdate: Partial<Omit<JournalEntry, "id">>) => {
    setJournalEntries((prev) => prev.map((entry) => (entry.id === id ? { ...entry, ...entryUpdate } : entry)))
  }
  ...
  const updateGoal = (id: string, goalUpdate: Partial<Omit<Goal, "id">>) => {
    setGoals((prev) => prev.map((goal) => (goal.id === id ? { ...goal, ...goalUpdate } : goal)))
  }
